...
fn draw_ship_vitals(ctx: &GameplayCtx<'_>, content: Rect) -> f32 {
    ...
    let contract_line = sim
        ...
        .unwrap_or_else(|| "Choose a charter in Voyage".to_owned());
    draw_ui_text_ex(
        "ACTIVE CONTRACT",
        content.x,
        y,
...
fn draw_ship_specs(ctx: &GameplayCtx<'_>, content: Rect, mut y: f32) -> f32 {
    ...
    for (label, value, color) in [
        ...
        (
            "CUSTODIAN CONDUCT",
            custodian_status(custodian_empathy).replace(" · PERCEIVED CONDUCT", ""),
...
fn draw_log_panel(ctx: &GameplayCtx<'_>, rect: Rect) {
    ...
    let content = Rect::new(rect.x + 18.0, rect.y + 40.0, rect.w - 36.0, rect.h - 58.0);
