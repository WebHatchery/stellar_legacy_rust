...
fn event_ids_are_globally_unique_across_every_authored_file() {
    let data = GameData::load().expect("toolkit-backed event registry");
    let mut seen = std::collections::HashSet::new();
    for (id, event) in data.events.iter() {
        assert!(!id.trim().is_empty(), "an embedded event has no id");
        assert!(seen.insert(id), "event id '{id}' is duplicated in the registry");
        assert!(!event.family.trim().is_empty(), "event '{id}' has no family");
    }
}

#[test]
fn cargo_game_and_save_versions_are_one_release_label() {
    let data = GameData::load().expect("toolkit-backed game config");
    assert_eq!(data.config.version, env!("CARGO_PKG_VERSION"));
}
