...
fn markdown(data: &GameData, aggregates: &[Aggregate]) -> String {
    let charter_count = data.contracts.len();
    let legacy_count = data.legacies.len();
    let loadout_count = Loadout::ALL.len();
    let policy_count = Policy::ALL.len();
    let cell_count = aggregates.len();
    let voyage_count: u32 = aggregates.iter().map(|aggregate| aggregate.runs).sum();
    let seed_count = aggregates.first().map_or(0, |aggregate| aggregate.runs);
    let mut out = format!(
        "# Stellar Legacy balance report\n\nGenerated from the current embedded registries. The full {cell_count}-cell matrix is in balance_matrix.csv; each cell contains {seed_count} seeded voyages.\n\n## Method\n\nThe matrix crosses {charter_count} charters, {legacy_count} legacies, {loadout_count} purchasable loadout profiles, and {policy_count} policies for {voyage_count} deterministic voyages. Charter deed, reputation, faction, and minimum-loadout gates are satisfied before launch so the report measures voyage balance rather than availability. The policies are: first authored choice; conservative material-outcome scoring with earlier maintenance; and objective-first scoring that strongly values mission progress.\n\nDisplayed 1× is the expected readable pace. Clock estimates use the configured seconds/month with truthful 1×/2×/3× multipliers. A human voyage budget adds 12 seconds per blocking decision and five minutes for drydock/preparation; these are planning interaction allowances, not simulation time.\n\n## Charter comparison\n\n| Charter | Runs | Complete | Partial | Pyrrhic | Failure | Extinction | Mean score | Mean objective | Mean net cr | Est. 1× / 2× / 3× |\n|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n");
    ...
    let complete: u32 = aggregates.iter().map(|aggregate| aggregate.complete).sum();
    let partial: u32 = aggregates.iter().map(|aggregate| aggregate.partial).sum();
    let extinction: u32 = aggregates.iter().map(|aggregate| aggregate.extinction).sum();
    let viable = complete + partial;
    let viable_rate = percentage(viable, voyage_count);
    let extinction_rate = percentage(extinction, voyage_count);
    out.push_str(&format!(
        "\n## Observations\n\nAcross {voyage_count} deterministic voyages, {viable_rate:.1}% returned in the Complete or Partial bands and {extinction_rate:.2}% ended in dynasty extinction. These figures describe the selected automated policies only; they are not human success-rate or pacing evidence.\n"
    ));
    out
}
...
mod tests {
    include!(concat!(
        ...
        "/tests/unit/simulation/balance/tests.rs"
    ));
}
